...
function createPlaceCard(place, type) {
  ...
  if (place.photo_ref && place.photo_ref.startsWith('LOCAL:')) {
     ...
     photoUrl = `https://placehold.co/400x300?text=${encodeURIComponent(cleanName)}`;
  } else {
     photoUrl = place.photo_ref 
      ? `/api/photo?ref=${encodeURIComponent(place.photo_ref)}` 
      : `/api/photo?keyword=${encodeURIComponent(place.name)}`; 
  }

  div.innerHTML = `
    <img src="${photoUrl}" class="card-img" alt="${place.name}" onerror="this.onerror=null;this.src='${FALLBACK_IMAGE}';">
    <div class="card-info">
      <h3>${place.name}</h3>
...
async function renderSuggestedDays(plan) {
  ...
  for (const dayPlan of plan) {
    ...
    // --- 1. BUILD THE GOOGLE MAPS ROUTE URL (Mobile Compatible) ---
    // We use standard 'dir' action with api=1
    let routeUrl = '#';
    if(places.length > 0) {
       const baseUrl = "https://www.google.com/maps/dir/?api=1";
       const destination = `&destination=${places[places.length-1].lat},${places[places.length-1].lng}`;
       
       if(places.length === 1) {
          // Just one place? Navigate to it.
          routeUrl = `${baseUrl}${destination}`;
       } else {
          // Multiple places? Create a full route with origin and waypoints
          const origin = `&origin=${places[0].lat},${places[0].lng}`;
          const waypointsList = places.slice(1, -1).map(p => `${p.lat},${p.lng}`).join('|');
          const waypoints = waypointsList ? `&waypoints=${waypointsList}` : '';
          routeUrl = `${baseUrl}${origin}${destination}${waypoints}&travelmode=driving`;
       }
    }
    ...
    places.forEach((p, index) => {
      ...
      // This will open navigation specifically to THIS place from user's current location
      const placeNavUrl = `https://www.google.com/maps/search/?api=1&query=${p.lat},${p.lng}&query_place_id=${p.id || ''}`;
      
      const photoUrl = p.photo_ref 
        ? `/api/photo?ref=${encodeURIComponent(p.photo_ref)}` 
        : `/api/photo?keyword=${encodeURIComponent(p.name)}`; 

      placesHtml += `
        <div class="itinerary-card">
          <div class="it-card-image-container">
             <img src="${photoUrl}" class="it-card-image" alt="${p.name}" onerror="this.onerror=null;this.src='${FALLBACK_IMAGE}';">
          </div>

...
app.get('/api/photo', async (req, res) => {
  ...
  if (ref && ref.startsWith('LOCAL:')) {
    ...
    const cleanName = localPath.split('/').pop().split('.')[0];
    return res.redirect(`https://placehold.co/400x300?text=${encodeURIComponent(cleanName)}`);
  }
  ...
  if (keyword) {
    const searchUrl = `https://tse4.mm.bing.net/th?q=${encodeURIComponent(keyword)}&w=400&h=300&c=7&rs=1&p=0`;
...
// 6. RESOLVE PLACE (NEW: Verification Logic)
